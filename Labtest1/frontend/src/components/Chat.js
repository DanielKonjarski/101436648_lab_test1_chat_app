...
function Chat() {
  ...
  const sendMessage = () => {
    if (message.trim()) {
      const newMessage = { from_user: 'Daniel', message };
      ...
      socket.emit('stopTyping', { room, user: 'Daniel' }); // Stop typing event
    }
  };
  ...
  const handleTyping = () => {
    if (!typing) {
      setTyping(true);
      socket.emit('userTyping', { room, user: 'Daniel' }); // Emit typing event
    }
    ...
    window.typingTimeout = setTimeout(() => {
      setTyping(false);
      socket.emit('stopTyping', { room, user: 'Daniel' }); // Emit stop typing event
  ...
  return (
    <div className="chat-container">
      ...
      {typing && <div className="typing-indicator">{typingUser} Daniel is typing...</div>}
